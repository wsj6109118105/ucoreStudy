...
page_init(void) {
    ...
    for (i = 0; i < npage; i ++) {     // 对所有物理空间实现占用标记
    ...
    for (i = 0; i < memmap->nr_map; i ++) {
        ...
        if (memmap->map[i].type == E820_ARM) {
            ...
            if (begin < end) {
                ...
                if (begin < end) {
                    init_memmap(pa2page(begin), (end - begin) / PGSIZE);    // 空闲标记
                }
            }
        }
    }
...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    ...
    if (!(*pdep & PTE_P)) {              // (2) check if entry is not present
        ...
        if(create) {          // (3) check if creating is needed, then alloc page for page table
            if((page=alloc_page())==NULL) {     // CAUTION: this page is used for page table, not for common data page
                return NULL;
            }
        }else {
            return NULL;
